"""
Convert a hinge-folding output dir into a HF DatasetDict matching the schema of
...
metadata["rotation_sequence"] — no LLM needed.

Usage:
    python build_hf_dataset.py --output-dir ../output_sft_100 --out ../hf_hingefolding_100
    python build_hf_dataset.py --output-dir ../output_sft_100 --out ../hf_hingefolding_100 --limit 10
    python build_hf_dataset.py ... --push-to-hub VCoTReasoningKU/HingeFolding-Instruct-Colored
"""
import argparse
import json
import random
...
QUESTION_PROMPT = (
    "Solve this hinge-folding puzzle. The image shows a chain of shapes connected "
    "by labeled hinges (A, B, C, …) on the left and the target folded shape on the right. "
    "At each step, identify which single hinge to rotate and by how many degrees (90, 180, or 270) "
    "to bring the chain closer to the target. Reason step by step, then state the hinge and angle."
)
...
def synth_answer(meta: dict) -> str:
    """
    Build 'A 90, B 180, C 270' from metadata['rotation_sequence'].
    Falls back to reconstructing from rotation_steps if needed.
    """
    seq = meta.get("rotation_sequence")
    if seq:
        return seq
    steps = meta.get("rotation_steps", [])
    return ", ".join(f"{s['hinge_id']} {s['angle']}" for s in steps)
...
def build_row(puzzle_dir: Path) -> dict | None:
    ...
    pid = meta.get("puzzle_id", puzzle_dir.name)
    # Normalise to a plain string id
    rid = f"hingefolding_{pid}" if not str(pid).startswith("hingefolding") else str(pid)
    ...
    for i, step in enumerate(steps_sorted):
        ...
        hinge_id = step.get("hinge_id", "") or ""
        angle = step.get("angle", "") or ""
    ...
    return {
        ...
        "options": None,
        "knowledge": "HingeFolding",
        "subknowledge": "HingeFolding",
    }
...
def main():
    ap = argparse.ArgumentParser(description="Build HF DatasetDict from hinge-folding output dir")
