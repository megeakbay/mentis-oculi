...
Usage:
    ...
    # Wait until done, polling every 60s:
    python batch_harvest.py --output-dir ../output_sft_100 --wait

    # Pass an explicit job name to override the recorded one:
...
def parse_response_text(raw: str) -> Dict[str, Any]:
    """Extract reasoning+hinge_id+angle or fall back to raw."""
    out: Dict[str, Any] = {}
    if raw is None:
        out["raw"] = None
        return out
    ...
    try:
        ...
        if isinstance(parsed, list) and parsed and isinstance(parsed[0], dict):
            parsed = parsed[0]
        if isinstance(parsed, dict) and "reasoning" in parsed and "hinge_id" in parsed and "angle" in parsed:
            hinge_id = parsed["hinge_id"]
            if isinstance(hinge_id, str):
                hinge_id = hinge_id.strip().upper()
            else:
                hinge_id = None
            try:
                angle = int(parsed["angle"])
                if angle not in VALID_ANGLES:
                    angle = None
            except (TypeError, ValueError):
                angle = None
            out["reasoning"] = parsed["reasoning"]
            out["hinge_id"] = hinge_id
            out["angle"] = angle
        else:
            out["raw"] = raw
...
def apply_results(output_dir: Path, results: Dict[str, str]) -> int:
    ...
    step entry in its cot_reasoning.json.
    """
    updated = 0
    ...
    for puzzle_dir, step_map in by_puzzle.items():
        ...
        for step in steps:
            ...
            idx = step.get("step")
            if idx in step_map:
                for stale in ("error", "gemini_response", "raw", "reasoning", "hinge_id", "angle", "response"):
                    step.pop(stale, None)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Harvest Gemini batch results for hinge-folding reasoning")
