"""
Convert a paper-fold output dir into a HF DatasetDict matching the schema of
...
metadata["correct_choice"] — no LLM needed.

Usage:
    python build_hf_dataset.py --output-dir ../output_sft_100 --out ../hf_paperfold_100
    python build_hf_dataset.py --output-dir ../output_sft_100 --out ../hf_paperfold_100 --limit 10
    python build_hf_dataset.py ... --push-to-hub VCoTReasoningKU/PaperFold-Instruct-Colored
"""
import argparse
import json
import random
...
QUESTION_PROMPT = (
    "Solve this paper-folding puzzle. The image shows a sequence of paper folds followed by a hole punch. "
    "Mentally unfold the paper step by step to determine where all holes will appear on the fully unfolded sheet. "
    "Choose the correct answer from the options shown (A–E). "
    "Reason step by step through the unfolding, then state your answer."
)
...
def synth_answer(meta: dict) -> str:
    """Return the correct choice label (e.g. 'C') from metadata."""
    return meta.get("correct_choice", "")
...
def build_row(puzzle_dir: Path) -> dict | None:
    ...
    rid = f"paperfold_{pid}" if not str(pid).startswith("paperfold") else str(pid)
    ...
    for i, step in enumerate(steps_sorted):
        ...
        fold_type = step.get("fold_type", "") or ""
    ...
    return {
        ...
        "options": ", ".join(meta.get("choices", ["A", "B", "C", "D", "E"])),
        "knowledge": "PaperFold",
        "subknowledge": "PaperFold",
    }
...
def main():
    ap = argparse.ArgumentParser(description="Build HF DatasetDict from paper-fold output dir")
