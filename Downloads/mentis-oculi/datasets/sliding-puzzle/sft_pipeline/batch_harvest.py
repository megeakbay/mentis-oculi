...
def parse_response_text(raw: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(parsed, list) and parsed and isinstance(parsed[0], dict):
            parsed = parsed[0]
        if isinstance(parsed, dict) and "reasoning" in parsed and "move" in parsed:
            move = parsed["move"]
...
def apply_results(output_dir: Path, results: Dict[str, str]) -> int:
    ...
    for puzzle_dir, step_map in by_puzzle.items():
        ...
        for step in steps:
            ...
            idx = step.get("step")
            if idx in step_map:
                for stale in ("error", "gemini_response", "raw", "reasoning", "move"):
                    step.pop(stale, None)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Harvest Gemini batch results for sliding-puzzle reasoning")
