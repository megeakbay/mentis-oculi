...
Usage:
    python judge_harvest.py --output-dir output_sft_50
    python judge_harvest.py --output-dir output_sft_50 --wait
"""
import argparse
import json
import os
...
def apply_results(output_dir: Path, results: Dict[str, str], model_name: str):
    ...
    for key, raw in results.items():
        try:
            # key: "{level}__{puzzle}__judge_step_{NN}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Harvest Gemini judge batch")
    ...
    if failures:
        ...
        for puzzle_dir, idx, flag, why in failures:
            rel = f"{puzzle_dir.parent.name}/{puzzle_dir.name}"
            snippet = (why or "").replace("\n", " ")[:200]
            print(f"  {rel} step {idx} {flag} — {snippet}")
